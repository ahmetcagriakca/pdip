...
``run()`` entry point so a future change to ``MessageBroker`` /
``TaskMessage`` that breaks the demo surfaces in CI rather than
when a contributor tries to follow the README.
"""
