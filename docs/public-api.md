...
| `pdip.observability` | `get_meter`, `get_tracer`, `inject_context`, `use_context` (lazy no-op-by-default OpenTelemetry helpers — `inject_context` / `use_context` form the cross-process W3C `traceparent` round-trip used by `ProcessManager` ↔ `Subprocess`; install `pdip[observability]` and set `PDIP_OBSERVABILITY_ENABLED=1` to emit) | [`pdip/observability/__init__.py`](../pdip/observability/__init__.py) |
