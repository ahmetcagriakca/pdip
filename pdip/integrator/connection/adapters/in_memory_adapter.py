...
from ..types.inmemory.base import InMemoryProvider
from ..types.sql.base import SqlProvider
from ...integration.domain.base import IntegrationBase


class InMemoryAdapter(ConnectionAdapter):
    @inject
    def __init__(self,
                 in_memory_provider: InMemoryProvider,
                 ):
        self.in_memory_provider = in_memory_provider

    def clear_data(self, integration: IntegrationBase) -> int:
        target_context = self.in_memory_provider.get_context_by_config(
    ...
    def get_source_data_count(self, integration: IntegrationBase) -> int:
        source_context = self.in_memory_provider.get_context_by_config(
